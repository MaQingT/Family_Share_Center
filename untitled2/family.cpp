...
int createFamily(QString name,QString password){
    ...
    if(db.open()){
        ...
        //创建视频表
        QString CreateVideoTable = QString("CREATE TABLE `%1`.`video` (`video_id` INT NOT NULL AUTO_INCREMENT,`video_name` NVARCHAR(45) NOT NULL,PRIMARY KEY (`video_id`),UNIQUE INDEX `video_id_UNIQUE` (`video_id` ASC));").arg(family_id);
